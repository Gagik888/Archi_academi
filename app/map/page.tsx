...
export default function MapPage() {
  ...
  async function checkUserByIp() {
    const savedIp = localStorage.getItem('archi_user_ip');
    if (!savedIp) { window.location.href = "/login"; return; }
    setUserIp(savedIp);

    const { data: profile } = await supabase.from('profiles').select('*').eq('id', savedIp).single();
    if (profile) {
  ...
  const resetCourse = () => {
    if (confirm("Хочешь сбросить текущий курс и выбрать новую тему?")) {
  ...
  const buyOrSelectSkin = async (skinId: string, price: number) => {
    ...
    if (unlockedSkins.includes(skinId)) {
      await supabase.from('profiles').update({ active_skin: skinId }).eq('id', userIp);
      setActiveSkin(skinId);
    } else {
      if (coins < price) return alert("Недостаточно ArchiCoins!");
      const newUnlocked = [...unlockedSkins, skinId];
      const newBalance = coins - price;
      
      // ВАЖНО: Обновляем базу
      const { error } = await supabase.from('profiles').update({ 
        coins: newBalance, unlocked_skins: newUnlocked, active_skin: skinId 
      }).eq('id', userIp);

      if (!error) {
        setCoins(newBalance); setUnlockedSkins(newUnlocked); setActiveSkin(skinId);
      } else {
  ...
  const checkCode = async () => {
    ...
    try {
      ...
      if (result.reward > 0 && userIp) {
        ...
        // Моментальное обновление в базе
        await supabase.from('profiles').update({ coins: newBalance }).eq('id', userIp);
        setCoins(newBalance);
      }
    } catch (e) {
  ...
  if (!data) return <div style={{ background: '#050505', minHeight: '100vh', padding: '50px', color: '#666' }}>Загрузка курса... <br/> <button onClick={() => window.location.href='/'}>На главную</button></div>;
    ...
    <div style={{ background: '#050505', color: '#fff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(5,5,5,0.9)', backdropFilter: 'blur(10px)', padding: '15px 25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 100, borderBottom: '1px solid #222' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <img src={`/archi-${activeSkin}.png`} style={{ width: '40px', height: '40px', objectFit: 'contain' }} />
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '15px' }}>
              {SKINS.map(skin => (
                <div key={skin.id} style={{ background: '#151515', padding: '20px', borderRadius: '25px', textAlign: 'center', border: activeSkin === skin.id ? `2px solid ${skin.color}` : '1px solid #222' }}>
                  <img src={`/archi-${skin.id}.png`} style={{ width: '60px', height: '60px', objectFit: 'contain', marginBottom: '10px' }} />
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', paddingTop: '120px', paddingBottom: '100px' }}>
        ...
        <h1 style={{ textAlign: 'center', color: skinColor, fontSize: '2.5rem', fontWeight: '900', marginTop: '20px' }}>{data.courseTitle}</h1>
        
        <div style={{ display: 'grid', gap: '20px', marginTop: '50px' }}>
          {data.levels.map((lvl: string, i: number) => (
